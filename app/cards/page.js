...
function PromotedBanner({ card }) {
  ...
  return (
    <a
      href={`/${card.username}`}
      style={{ display: "block", textDecoration: "none", position: "relative", borderRadius: 20, overflow: "hidden", marginBottom: 20 }}
    >
      {/* Background */}
      <div style={{
        ...
        {card.image_url && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={card.image_url} alt={card.name} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: card.image_position || "center" }} />
        )}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.2) 100%)" }} />

        {/* Promoted badge */}
        <div style={{ position: "absolute", top: 12, left: 12, display: "flex", alignItems: "center", gap: 5, background: "rgba(236,72,153,0.9)", backdropFilter: "blur(8px)", borderRadius: 100, padding: "4px 10px" }}>
          <span style={{ color: "#fff", fontSize: 10, fontWeight: 800, letterSpacing: "0.08em" }}>⚡ PROMOTED</span>
        </div>

        {/* Content */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 16px 16px" }}>
          <p style={{ color: "#fff", fontWeight: 700, fontSize: 17, lineHeight: 1.2, marginBottom: 3, textShadow: "0 2px 10px rgba(0,0,0,0.5)" }}>{card.name}</p>
          {card.bio && <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 12, lineHeight: 1.4, marginBottom: 10, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{card.bio}</p>}
          {waHref && (
            <a href={waHref} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
              style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#25D366", color: "#fff", borderRadius: 10, padding: "8px 16px", fontWeight: 700, fontSize: 13, textDecoration: "none", boxShadow: "0 3px 14px rgba(37,211,102,0.45)" }}>
...
export default function CardsPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(1)
      .maybeSingle()
      .then(({ data }) => setPromotedCard(data));
  }, []);
  ...
  return (
    <main className="min-h-screen" style={{ background: "var(--bg-page)", overflowX: "hidden" }}>
      ...
      <div className="relative z-10 max-w-lg mx-auto px-5 pt-14 pb-28">
        ...
        {/* Promoted card — full width above the grid */}
        {promotedCard && <PromotedBanner card={promotedCard} />}
